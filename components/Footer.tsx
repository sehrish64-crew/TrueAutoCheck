...
const footerLinks: Array<{ key: string; href: string }> = []
        ...
        <div className="border-b border-gray-800 pb-4 sm:pb-5">
          <div className="flex flex-col items-center gap-3 sm:gap-4">
            <p className="text-center text-gray-400 text-xs sm:text-xs md:text-sm lg:text-sm leading-tight sm:leading-relaxed" style={{ fontSize: '11px' }}>
              <span className="block mb-1.5 sm:mb-2 md:mb-0 md:inline">
                {t('footer_copyright_notice')} {new Date().getFullYear()} © Auto Facts Check. {t('footer_use_notice')}
              </span>

              {footerLinks.map((link, idx) => (
                <span key={link.key} className="inline">
                  <Link href={link.href} className="text-blue-400 hover:text-blue-300 transition-colors">
                    <u>{t(link.key)}</u>
                  </Link>
              ...
              <span className="block md:inline">
                {t('footer_company_note')} <u>{t('footer_contact_email')}</u>
              </span>
            </p>
          </div>
        </div>
