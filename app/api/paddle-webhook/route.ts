...
export async function POST(request: NextRequest) {
  try {
    ...
    if (alert === 'subscription_created' || alert === 'payment_succeeded') {
      const customerEmail = params.email || params.customer_email || params.payer_email
      const userName = params.user_name || params.customer_name || ''
      const plan = params.plan_name || params.product_name || ''

      // Send customer confirmation (optional — Paddle also sends their own)
      if (customerEmail) {
        const html = `<p>Hi ${userName || 'Customer'},</p>
          <p>Thanks for your purchase of <strong>${plan}</strong>. Your payment was received.</p>
          <p>If you have any questions, reply to this email.</p>`
        try {
          await sendEmail(customerEmail, 'Your purchase confirmation', html)
        } catch (err) {
      ...
      if (admin) {
        const html = `<p>New ${alert} event received.</p>
          <p>Customer: ${customerEmail || 'N/A'}</p>
          <p>Plan/Product: ${plan}</p>
          <pre>${JSON.stringify(params, null, 2)}</pre>`
        try {
          await sendEmail(admin, `Paddle webhook — ${alert}`, html)
        } catch (err) {
